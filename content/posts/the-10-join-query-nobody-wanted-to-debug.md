...
description = "When a production bug can only be fixed by someone who remembers a past incident, the abstraction is already broken."
tags = ["backend", "sql", "architecture", "debugging", "maintainability"]
categories = ["engineering", "architectrure"]
...
There were no race conditions, no partial failures, no corrupt data. The SQL was valid. The results looked plausible. The system failed in a quieter way.
...
When a system can only be debugged by people who remember its history, the system has started to depend on institutional memory — and that dependency is invisible until the day it isn’t there.
...
The query didn’t exist by accident. It optimized for fewer round trips, simpler pagination, and a single place to apply filters.
...
- Even after fixing the bug, we weren’t confident it was the _only_ bug.
...
But the rule —  
_“bulk-weight items never appear in standard listings”_ —  
is now enforced by structure, not a conditional branch.

No `OR` clause can accidentally violate it.

There is some extra orchestration. A few more queries. That tradeoff was worth it.
...
This preserves pagination correctness while dramatically reducing query complexity — and makes cross-flow leakage impossible.
